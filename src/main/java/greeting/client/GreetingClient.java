...
public class GreetingClient {
  private static void doGreet(ManagedChannel channel) {
    ...
    GreetingResponse response =
        stub.greet(GreetingRequest.newBuilder().setFirstName("Clement").build());
  ...
  private static void doGreetManyTimes(ManagedChannel channel) {
    ...
    GreetingServiceGrpc.GreetingServiceBlockingStub stub =
        GreetingServiceGrpc.newBlockingStub(channel);
    stub.greetingManyTimes(GreetingRequest.newBuilder().setFirstName("Clement").build())
        .forEachRemaining(
            response -> {
  ...
  private static void doLongGreet(ManagedChannel channel) throws InterruptedException {
    ...
    StreamObserver<GreetingRequest> stream =
        stub.longGreet(
  ...
  private static void doGreetEveryone(ManagedChannel channel) throws InterruptedException {
    ...
    StreamObserver<GreetingRequest> streamObserver =
        stub.greetEveryone(
            new StreamObserver<GreetingResponse>() {
              @Override
    ...
    Arrays.asList("Clement", "Levi", "Tester")
        .forEach(
            name -> {
